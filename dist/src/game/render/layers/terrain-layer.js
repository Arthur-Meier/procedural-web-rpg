...
export class TerrainLayer extends WorldRenderLayerBase {
    drawGround() {
        ...
        this.ctx.fillRect((bounds.minX - 3) * PIXELS_PER_METER, (bounds.minY - 3) * PIXELS_PER_METER, (bounds.maxX - bounds.minX + 6) * PIXELS_PER_METER, (bounds.maxY - bounds.minY + 6) * PIXELS_PER_METER);
            ...
            for (let y = 0; y < 12; y += 1) {
                for (let x = 0; x < 12; x += 1) {
                    ...
                    if (patch > 0.82) {
                        ...
                        this.ctx.beginPath();
                        this.ctx.ellipse((worldX + 1) * PIXELS_PER_METER, (worldY + 1) * PIXELS_PER_METER, cellPixels * 0.45, cellPixels * 0.28, 0, 0, Math.PI * 2);
                        this.ctx.fill();
                    }
                    if (dirt > 0.44 && dirt < 0.56) {
                        this.ctx.fillStyle = "rgba(113, 88, 50, 0.2)";
                        this.ctx.beginPath();
                        this.ctx.ellipse(screenX + cellPixels * 0.48, screenY + cellPixels * 0.72, cellPixels * 0.34, cellPixels * 0.12, -0.28, 0, Math.PI * 2);
                        this.ctx.fill();
                    }
                    if (detail > 0.72) {
