    ...
    drawBurnEffects() {
        for (const effect of this.burnEffects) {
            ...
            for (let flame = 0; flame < 3; flame += 1) {
                ...
                const flameGradient = this.ctx.createRadialGradient(flameX, flameY - flameHeight * 0.32, flameWidth * 0.08, flameX, flameY, flameHeight * 0.78);
    ...
    drawEnemyDeathEffects() {
        for (const effect of this.enemyDeathEffects) {
            ...
            if (effect.kind === Mage.kind) {
                ...
                this.ctx.beginPath();
                this.ctx.ellipse(x, y + radius * 0.45, radius * (1 + progress * 0.45), radius * Math.max(0.16, 0.52 - progress * 0.26), 0, 0, Math.PI * 2);
                this.ctx.fill();
            }
            this.ctx.restore();
        }
