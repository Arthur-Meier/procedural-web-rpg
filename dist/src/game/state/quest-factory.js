...
function createQuestState(templateIndex, rotation) {
    ...
    const descriptionSuffix = normalizedRotation === 0 ? "" : " Uma nova leva de criaturas apareceu e precisa ser contida.";
    return {
...
export function createNextQuestState(previousQuest) {
    return createQuestState(resolveTemplateIndex(previousQuest.templateId, 0), previousQuest.rotation + 1);
}
export function hydrateQuestBoardState(quests, now = Date.now()) {
    ...
    return quests.map((quest, index) => {
        ...
        const completedAt = completed && typeof quest?.completedAt === "number" ? quest.completedAt :
            completed ? now : null;
        const refreshAt = completed && typeof quest?.refreshAt === "number" ? quest.refreshAt :
...
export function getQuestRefreshRemaining(quest, now = Date.now()) {
