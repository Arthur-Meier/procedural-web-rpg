...
import type { CameraState, Drop, EnemyEntity, Player, Projectile, RespawnEntry, UiState } from "./game/types.js";
import type {
  BurnEffect,
  EffectParticle,
  EnemyDeathEffect,
...
class Game {
  ...
  hud: HTMLElement; hudTop: HTMLElement; hudSide: HTMLElement; hudBottom: HTMLElement; hudMessage: HTMLElement;
  overlayVeil: HTMLElement; titlePanel: HTMLElement; titleLoadPanel: HTMLElement; pausePanel: HTMLElement; gameOverPanel: HTMLElement;
  mapPanel: HTMLElement; statsPanel: HTMLElement; inventoryPanel: HTMLElement; questPanel: HTMLElement;
  statsOverview: HTMLElement; statsAllocation: HTMLElement; inventoryCharacter: HTMLElement; inventoryEquipment: HTMLElement; craftingWorkbench: HTMLElement; craftingOutput: HTMLElement; inventoryCapacity: HTMLElement; inventoryPanelGrid: HTMLElement;
  questBoardList: HTMLElement; titleLoadSlots: HTMLElement; pauseSaveSlots: HTMLElement; pauseLoadSlots: HTMLElement; gameOverLoadSlots: HTMLElement;
  uiState: UiState; mapOpen: boolean; statsOpen: boolean; inventoryOpen: boolean; questBoardOpen: boolean; showingTitleLoads: boolean;
  player: Player; world: World; enemies: EnemyEntity[]; projectiles: Projectile[]; drops: Drop[]; particles: EffectParticle[];
  burnEffects: BurnEffect[]; floatingTexts: FloatingTextEffect[];
  pendingSpellCasts: PendingSpellCast[]; enemyDeathEffects: EnemyDeathEffect[]; playerAuraEffects: PlayerAuraEffect[]; pendingRespawns: RespawnEntry[];
  seed: number; enemyIdCounter: number; dropIdCounter: number; projectileIdCounter: number; effectIdCounter: number; camera: CameraState;
  dayNightSystem: DayNightSystem;
  dayCount: number;
  message: string; messageTimer: number; lastTimestamp: number; mapPanX: number; mapPanY: number; mapDragging: boolean; mapDragLastX: number; mapDragLastY: number;
  quests: QuestState[]; input: InputManager; worldRenderer: WorldRenderer; combatSystem: CombatSystem; movementSystem: MovementSystem;
  progressionSystem: ProgressionSystem; sessionSystem: SessionSystem; spawnSystem: SpawnSystem; questSystem: QuestSystem;
  overlayController: OverlayController; gameLoop: GameLoop;
  ...
  renderInventoryPanel(): void {
    ...
    renderInventoryPanelView(
      ...
      this.player,
      (weaponId) => this.progressionSystem.equipWeapon(weaponId),
      (recipeId) => this.progressionSystem.craftWeapon(recipeId)
    );
  }
  ...
  renderStatsPanel(): void {
    ...
    renderStatsPanelView(
      ...
      this.player,
      (statKey) => this.progressionSystem.spendStatPoint(statKey)
    );
  }
  ...
  updateHud(): void {
    ...
    updateHudView(
      ...
      this.enemies,
      this.message,
      this.movementSystem.isNearGuideNpc(),
      this.questSystem.getActiveQuest(),
      this.dayCount
    );
  }
  ...
  setMessage(text: string, duration = 2.6): void {
    this.message = text;
