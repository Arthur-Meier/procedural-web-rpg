...
interface DayNightKeyframeDefinition {
  section: "day" | "night";
  progress: number;
  phase: DayNightPhase;
  atmosphereTop: string;
...
interface ResolvedDayNightKeyframe extends Omit<DayNightKeyframeDefinition, "section" | "progress"> {
  time: number;
}

export const DEFAULT_DAY_NIGHT_CONFIG: DayNightConfig = Object.freeze({
...
const DAY_NIGHT_KEYFRAMES: readonly DayNightKeyframeDefinition[] = Object.freeze([
  {
    section: "day",
    progress: 0,
    phase: "dawn",
...
export class DayNightSystem {
  ...
  constructor(private readonly config: DayNightConfig = DEFAULT_DAY_NIGHT_CONFIG) {
  ...
  reset(elapsedSeconds = this.config.initialTimeSeconds): void {
  ...
  createSnapshot(): DayNightSnapshot {
    return {
  ...
  getLightingState(): DayNightLightingState {
    ...
    const nightProgress =
      dayRatio < 1 && normalizedTime > dayRatio ? clamp((normalizedTime - dayRatio) / (1 - dayRatio), 0, 1) : 0;
  ...
  private resolveKeyframes(): ResolvedDayNightKeyframe[] {
    const dayRatio = this.config.dayDurationSeconds / this.totalDurationSeconds;
  ...
  private resolveSegment(
    keyframes: ResolvedDayNightKeyframe[],
    normalizedTime: number
  ): [ResolvedDayNightKeyframe, ResolvedDayNightKeyframe, number] {
