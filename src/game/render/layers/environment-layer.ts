...
export class EnvironmentLayer extends WorldRenderLayerBase {
  ...
  private drawTree(object: BreakableObject): void {
  ...
  private drawRock(object: BreakableObject): void {
    const x = object.x * PIXELS_PER_METER;
    const y = object.y * PIXELS_PER_METER;
    const radius = object.radius * PIXELS_PER_METER;
  ...
  private drawCrate(object: BreakableObject): void {
    const x = object.x * PIXELS_PER_METER;
    const y = object.y * PIXELS_PER_METER;
  ...
  private drawItemDropVisual(drop: Extract<Drop, { kind: "item" }>, x: number, y: number): void {
    if (drop.itemId === "wood") {
      this.ctx.fillStyle = "rgba(0, 0, 0, 0.16)";
      this.ctx.beginPath();
