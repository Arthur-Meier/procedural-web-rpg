...
export class EntityLayer extends WorldRenderLayerBase {
  ...
  private drawSlimeEnemy(enemy: EnemyEntity, x: number, y: number, radius: number): void {
  ...
  private drawMageEnemy(enemy: EnemyEntity, x: number, y: number, radius: number): void {
    const breathing = 1 + Math.sin(this.lastTimestamp / 180 + enemy.id) * 0.04;
    const facingAngle =
      enemy.dashDirection.x || enemy.dashDirection.y ? Math.atan2(enemy.dashDirection.y, enemy.dashDirection.x) : Math.PI / 2;
  ...
  private drawSwordSwingEffect(x: number, y: number, radius: number): void {
