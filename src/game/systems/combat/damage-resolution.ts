import {
  ...
  PLAYER_ATTACK_RANGE,
  PLAYER_ATTACK_SWING_TIME,
  PLAYER_INVULNERABILITY,
  PLAYER_MAGIC_COOLDOWN,
  PROJECTILE_RADIUS,
  PROJECTILE_RANGE,
  PROJECTILE_SPEED
} from "../../constants.js";
...
import type { CombatActions, CombatSystemHost, DamageSource } from "./types.js";

export class CombatDamageSystem {
  constructor(
    private readonly host: CombatSystemHost,
    private readonly actions: Pick<CombatActions, "killEnemy" | "spawnBreakableDrops" | "startSpellCast">
  ) {}
  ...
  performSwordAttack(): void {
    ...
    for (const enemy of this.host.enemies) {
      ...
      const withinArc =
        Math.abs(angleDifference(this.host.player.facingAngle, angleBetween(this.host.player.x, this.host.player.y, enemy.x, enemy.y))) <=
        Math.PI / 2;
    ...
    for (const object of objects) {
      ...
      const withinArc =
        Math.abs(angleDifference(this.host.player.facingAngle, angleBetween(this.host.player.x, this.host.player.y, object.x, object.y))) <=
        Math.PI / 2;
  ...
  damagePlayer(amount: number, _source: DamageSource = "direct"): void {
  ...
  damageEnemy(enemy: EnemyEntity, amount: number, _source: DamageSource = "direct"): void {
    if (enemy.dead) {
      return;
    }
