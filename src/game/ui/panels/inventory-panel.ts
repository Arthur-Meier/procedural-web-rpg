...
export function renderInventoryPanel(
  refs: InventoryPanelRefs,
  player: Player,
  onEquipWeapon: WeaponEquipHandler,
  _onCraftWeapon: CraftWeaponHandler
): void {
...
function renderCharacterPreview(refs: InventoryPanelRefs, player: Player): void {
...
function renderArmorSlots(refs: InventoryPanelRefs): void {
...
function renderWorkbench(refs: InventoryPanelRefs): void {
...
function renderOutputSlot(refs: InventoryPanelRefs): void {
...
function renderInventorySlots(
  ...
  player.inventory.forEach((slot, index) => {
    ...
    if (!slot) {
      ...
      const modifier =
        weapon.slot === "sword" ? `Fisico ${weapon.physicalMultiplier}x` : `Magico ${weapon.magicMultiplier}x`;
      const isEquipped =
        (weapon.slot === "sword" && player.weapons.sword.id === weapon.id) ||
...
function bindInventoryActivation(element: HTMLElement, onActivate: () => void): void {
  element.tabIndex = 0;
