...
import type { WorldMapLayout } from "../app-types.js";
import type { Player } from "../types.js";
import { World } from "../world.js";

interface WorldMapLayoutArgs {
  world: World;
  player: Player;
...
interface RenderWorldMapArgs extends WorldMapLayoutArgs {
  mapContext: CanvasRenderingContext2D;
  seed: number;
}

export function getWorldMapLayout(args: WorldMapLayoutArgs): WorldMapLayout | null {
  ...
  const fitCell = Math.min(
    (viewportWidth - padding * 2) / Math.max(1, discoveredWidth),
    (viewportHeight - padding * 2) / Math.max(1, discoveredHeight)
  );
...
export function renderWorldMap(args: RenderWorldMapArgs): WorldMapLayout | null {
  ...
  mapContext.lineTo(
    playerX + Math.cos(player.facingAngle) * Math.max(10, layout.cell * 0.34),
    playerY + Math.sin(player.facingAngle) * Math.max(10, layout.cell * 0.34)
  );
  mapContext.stroke();
...
export function centerMapOnPlayer(args: WorldMapLayoutArgs): { x: number; y: number } | null {
  const layout = getWorldMapLayout(args);
  if (!layout) {
    return null;
  }
