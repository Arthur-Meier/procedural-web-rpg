...
import type { OverlayPanel } from "../app-types.js";
import type { CameraState, Player, UiState } from "../types.js";
import { World } from "../world.js";

export interface OverlayControllerHost {
...
interface OverlayControllerCallbacks {
  ...
  renderQuestBoard(): void;
  setMessage(text: string, duration?: number): void;
}

export class OverlayController {
  constructor(
    private readonly host: OverlayControllerHost,
    private readonly callbacks: OverlayControllerCallbacks
  ) {}

  bindUi(): void {
    getRequiredElement<HTMLButtonElement>("newGameButton").addEventListener("click", () => this.callbacks.newGame());
    getRequiredElement<HTMLButtonElement>("showLoadButton").addEventListener("click", () => {
      this.host.showingTitleLoads = true;
      this.callbacks.refreshSlotLists();
      this.syncUi();
    });
    getRequiredElement<HTMLButtonElement>("backToTitleButton").addEventListener("click", () => {
      this.host.showingTitleLoads = false;
      this.syncUi();
    });
    getRequiredElement<HTMLButtonElement>("exitButton").addEventListener("click", () => {
      window.close();
      this.callbacks.setMessage("Se o navegador nao fechar, feche a aba manualmente.", 4);
    });

    getRequiredElement<HTMLButtonElement>("resumeButton").addEventListener("click", () => this.callbacks.resumeGame());
    getRequiredElement<HTMLButtonElement>("backToMenuButton").addEventListener("click", () => this.callbacks.goToTitle());
    getRequiredElement<HTMLButtonElement>("retryButton").addEventListener("click", () => this.callbacks.newGame());
    getRequiredElement<HTMLButtonElement>("gameOverMenuButton").addEventListener("click", () => this.callbacks.goToTitle());
  ...
  renderWorldMapIfOpen(): void {
    if (this.host.mapOpen) {
      this.renderWorldMap();
    }
  }

  private setActiveOverlayPanel(panel: OverlayPanel): void {
    if (!this.host.player || this.host.uiState !== "playing") {
      return;
    }
